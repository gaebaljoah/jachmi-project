...
<div id="body-wrapper">
	<div id="body-content">
	<h2 style="text-align: center; margin-bottom: 10px;">내신청서확인</h2>
	<hr class="my-hr3">
	<!-- 왼쪽 내목록 카테고리 -->
	  <div id="box"> 
	      <ul>
      ...
      <tbody>
		...
		<td><a href="/seller/get?pageNum=${pageMaker.cri.pageNum }&amount=${pageMaker.cri.amount }&s_num=${board.s_num }" title="상세페이지로 이동합니다.">
		<c:out value="${board.s_title}"/></a>
		</td>
		<td><c:out value="${board.s_category}"/></td>
		<td><c:out value="${board.s_title}"/></td>
		<td><c:out value="${board.s_person}"/></td>
		<td><c:out value="${board.s_startday}~${board.s_endday}"/></td>
		</tr>
		</c:forEach>
      </tbody>
    </table>
